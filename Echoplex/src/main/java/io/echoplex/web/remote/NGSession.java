...
class NGSession extends Thread {
	 ...
	 * @param server The CommandServer we're working for
	 */
	NGSession(NGSessionPool sessionPool, CommandServer server) {
		super();
	...
	public void run() {
		...
		while (socket != null) {
			try {
				...
				List<String> remoteArgs = new java.util.ArrayList<String>();
			...
			//	((ThreadLocalPrintStream) System.err).init(err);
				((io.echoplex.web.remote.ThreadLocalInputStream) System.in).init(in);
