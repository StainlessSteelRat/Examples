...
import io.echoplex.web.remote.Alias;
...
public class HaloAlias {
	...
	public static void nailMain(NGContext context) throws ClassNotFoundException {
		...
		CommandServer server = context.getServer();
		
		if (args.length == 0) {
			Set<Alias> aliases = server.getAliasManager().getAliases();
			
			// let's pad this nicely.  first, find the longest alias
			// name.  then pad the others to that width.
			int maxAliasLength = 0;
			int maxClassnameLength = 0;
			for (Iterator<Alias> i = aliases.iterator(); i.hasNext();) {
			...
			for (Iterator<?> i = aliases.iterator(); i.hasNext();) {
				...
				context.out.println(padl(alias.getName(), maxAliasLength) 	+ "\t" + alias.getDescription());
